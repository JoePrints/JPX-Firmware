...
static void lcd_preheat_menu()
{
  ...
  if (farm_mode) {
	  ...
	  MENU_ITEM_FUNCTION_P(PSTR("ASA  -  " STRINGIFY(ABS_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(ABS_PREHEAT_HPB_TEMP)), lcd_preheat_abs);
	  MENU_ITEM_FUNCTION_P(PSTR("CPE -  " STRINGIFY(HIPS_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(HIPS_PREHEAT_HPB_TEMP)), lcd_preheat_hips);
	  MENU_ITEM_FUNCTION_P(PSTR("FLEX -  " STRINGIFY(FLEX_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(FLEX_PREHEAT_HPB_TEMP)), lcd_preheat_flex);
    MENU_ITEM_FUNCTION_P(PSTR("NOZZLE -  " STRINGIFY(PP_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PP_PREHEAT_HPB_TEMP)), lcd_preheat_pp);
...
MENU_ITEM_SUBMENU_P(PSTR("ASA  -  " STRINGIFY(ABS_PREHEAT_HOTEND_TEMP)),mFilamentItem_ABS);
MENU_ITEM_SUBMENU_P(PSTR("CPE -  " STRINGIFY(HIPS_PREHEAT_HOTEND_TEMP)),mFilamentItem_HIPS);
MENU_ITEM_SUBMENU_P(PSTR("NOZZLE -  " STRINGIFY(PP_PREHEAT_HOTEND_TEMP)),mFilamentItem_PP);
...
void lcd_wizard(WizState state)
{
	...
	while (!end) {
		...
		case S::Run: //Run wizard?
			wizard_active = true;
			wizard_event = lcd_show_multiscreen_message_yes_no_and_wait_P(_i("Hi, I am your Original Payne R&D i3 printer. Would you like me to guide you through the setup process?"), false, true);////MSG_WIZARD_WELCOME c=20 r=7
			if (wizard_event) {
				state = S::Restore;
