...
static void lcd_preheat_nylon()
{
  setTargetHotend0(NYLON_PREHEAT_HOTEND_TEMP);
  if (!wizard_active) setTargetBed(NYLON_PREHEAT_HPB_TEMP);
  fanSpeed = NYLON_PREHEAT_FAN_SPEED;
  lcd_return_to_status();
  setWatch(); // heater sanity check timer
  if (wizard_active) lcd_wizard(WizState::Unload);
}

static void lcd_preheat_cool()
{
  setTargetHotend0(COOL_PREHEAT_HOTEND_TEMP);
  if (!wizard_active) setTargetBed(COOL_PREHEAT_HPB_TEMP);
  fanSpeed = COOL_PREHEAT_FAN_SPEED;
  lcd_return_to_status();
  setWatch(); // heater sanity check timer
  if (wizard_active) lcd_wizard(WizState::Unload);
}
...
{
  setTargetHotend0(NOZZLE_PREHEAT_HOTEND_TEMP);
  if (!wizard_active) setTargetBed(NOZZLE_PREHEAT_HPB_TEMP);
  fanSpeed = NOZZLE_PREHEAT_FAN_SPEED;
  lcd_return_to_status();
  setWatch(); // heater sanity check timer
  if (wizard_active) lcd_wizard(WizState::Unload);
}
...
static void lcd_preheat_menu()
{
  ...
  if (farm_mode) {
    ...
    MENU_ITEM_FUNCTION_P(PSTR("FLEX -  " STRINGIFY(FLEX_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(FLEX_PREHEAT_HPB_TEMP)), lcd_preheat_flex);
	  MENU_ITEM_FUNCTION_P(PSTR("NYLON  -  " STRINGIFY(NYLON_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(NYLON_PREHEAT_HPB_TEMP)), lcd_preheat_nylon);
	  MENU_ITEM_FUNCTION_P(PSTR("NOZZLE -  " STRINGIFY(NOZZLE_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(NOZZLE_PREHEAT_HPB_TEMP)), lcd_preheat_hips);
...
static void mFilamentItem_NYLON()
{
bFilamentPreheatState=false;
mFilamentItem(NYLON_PREHEAT_HOTEND_TEMP,NYLON_PREHEAT_HPB_TEMP);
}

static void mFilamentItem_NOZZLE()
{
bFilamentPreheatState=false;
mFilamentItem(NOZZLE_PREHEAT_HOTEND_TEMP,NOZZLE_PREHEAT_HPB_TEMP);
}

static void mFilamentItem_COOL()
{
bFilamentPreheatState=false;
mFilamentItem(COOL_PREHEAT_HOTEND_TEMP,COOL_PREHEAT_HPB_TEMP);
}
...
void mFilamentMenu()
{
    ...
    MENU_ITEM_SUBMENU_P(PSTR("ASA  -  " STRINGIFY(ASA_PREHEAT_HOTEND_TEMP)),mFilamentItem_ASA);
    MENU_ITEM_SUBMENU_P(PSTR("FLEX -  " STRINGIFY(FLEX_PREHEAT_HOTEND_TEMP)),mFilamentItem_FLEX);
    MENU_ITEM_SUBMENU_P(PSTR("NYLON  -  " STRINGIFY(NYLON_PREHEAT_HOTEND_TEMP)),mFilamentItem_NYLON);
    MENU_ITEM_SUBMENU_P(PSTR("NOZZLE -  " STRINGIFY(NOZZLE_PREHEAT_HOTEND_TEMP)),mFilamentItem_NOZZLE);
...
void lcd_wizard(WizState state)
{
	...
	while (!end) {
		...
		case S::Run: //Run wizard?
			...
			wizard_active = true;
			wizard_event = lcd_show_multiscreen_message_yes_no_and_wait_P(_i("Hi, I am your Original Payne R&D printer. Would you like me to guide you through the setup process?"), false, true);////MSG_WIZARD_WELCOME c=20 r=7
			if (wizard_event) {
				state = S::Restore;
...
static bool selftest_irsensor()
{
    ...
    uint8_t progress;
    {
        ...
        setTargetHotend(NYLON_PREHEAT_HOTEND_TEMP,active_extruder);
